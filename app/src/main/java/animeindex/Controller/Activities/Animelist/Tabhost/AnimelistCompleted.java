...
public class AnimelistCompleted extends ListActivity {
        ...
        switch (requestCode){
            case EDIT_REQUEST_CODE:
                if(resultCode == RESULT_OK) {
                    Log.d("asd", "animelist completed");
                    editAnimelist((Animelist) data.getSerializableExtra(ANIME_COMPLETED_TAG));
                    setAdapter();
                }
                else {
                    Log.d("asd", "BOTOK");
    ...
    public void onClick(ListView parent,
        ...
        ArrayList<Animelist> col = completedArray;
        ...
        intent.putExtra(ANIME_COMPLETED_TAG, col.get(position));

        getParent().startActivityForResult(intent, EDIT_REQUEST_CODE);
    }
}
